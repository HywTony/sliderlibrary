...
public class SliderLayout extends RelativeLayout {
     ...
     */
    private int mTransformerSpan = 10;
    ...
    private int indicatorSelectColor = 0;
    private int indicatorUnSelectColor = 0;
    /**
     * 是否可以左右滑动拖拽 by huangyouwei 2015.3.24
     *
     * 当使用当前该控件的时候，如果只有一项，则可调用方法setIsCanDrag（false）方法。控制他不能滑动。
     */
    ...
    public SliderLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        mTransformerSpan = attributes.getInteger(R.styleable.SliderLayout_pager_animation_span, 10);
        mTransformerSpan=10;
        ...
        mViewPager.setOnTouchListener(new OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                switch (action) {
                    //滑动ViewPager引起swiperefreshlayout刷新的冲突,对swipeRefreshLayout控制。
    ...
    /**
     * 重新赋值时,先将item切换时间置为10ms,而后再恢复到正常值
     */
    ...
    /**
     * 之前版本存在一个bug:第一次打开banner,会先闪过第一张
     * 解决方法:让第一张快速划过,再将item切换的时间恢复到正常即可
     */
    private void updateTransformerSpan() {
        if (mTransformerSpan == 10) {
            mTransformerSpan = 1100;
            setSliderTransformDuration(mTransformerSpan, null);
        }
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                updateTransformerSpan();
                if (!mIsCanDrag) {//不能手滑
                    mViewPager.setIsCanDrag(mIsCanDrag);
                } else {
                    mViewPager.setIsCanDrag(mIsCanDrag);
                    pauseAutoCycle();
                }
    ...
    public void setPresetIndicator(PresetIndicators presetIndicator) {
        ...
        if(indicatorSelectColor==0)
            indicatorSelectColor= Color.rgb(255,255,255);
        if(indicatorUnSelectColor==0)
            indicatorUnSelectColor=Color.argb(33,255,255,255);
        pagerIndicator.setDefaultIndicatorColor(indicatorSelectColor,indicatorUnSelectColor);
